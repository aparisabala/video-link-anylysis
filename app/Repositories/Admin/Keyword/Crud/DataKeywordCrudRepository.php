...
use App\Http\Requests\Admin\Keyword\Crud\ValidateUpdateDataKeyword;
...
use Auth;
use DB;
class  DataKeywordCrudRepository extends BaseRepository implements IDataKeywordCrudRepository {

    use BaseTrait;
    public function __construct() {
        $this->LoadModels(['DataKeyword']);
    }
    ...
    {
       return $this->getPageDefault(model: $this->DataKeyword, id: $id);
    }
    ...
    {
        $model = DataKeyword::query();
        $this->saveTractAction(
            $this->getTrackData(
                title: 'DataKeyword was viewed by '.$request?->auth?->name.' at '.Carbon::now()->format('d M Y H:i:s A'),
                request: $request,
                onlyTitle: true
            )
        );
    ...
    {   
        ...
        try {
            DataKeyword::create([
                ...$request->all(),
                //'serial' => $this->facSrWc($this->DataKeyword)
            ]);
            $response['extraData'] = ['inflate' => pxLang($request->lang,'','common.action_success') ];
            $this->saveTractAction($this->getTrackData(title: "DataKeyword was created by ".$request?->auth?->name,request: $request));
            DB::commit();
            return $this->response(['type' => 'success', 'data' => $response]);
        } catch (\Exception $e) {
            DB::rollback();
            $this->saveError($this->getSystemError(['name' => 'DataKeyword_store_error']), $e);
    ...
    public function update($request,$id) : JsonResponse
    {
        ...
        if($row->isDirty()){
            $validator = Validator::make($request->all(), (new ValidateUpdateDataKeyword())->rules($request,$row));
            if ($validator->fails()) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: " DataKeyword ".$row?->name.' was updated by '.$request?->auth?->name,request: $request, row: $rowRef, type: 'to'));
                DB::commit();
                return $this->response(['type' => 'success','data' => $data]);
            } catch (\Exception $e) {
                DB::rollback();
                $this->saveError($this->getSystemError(['name'=>'DataKeyword_update_error']), $e);
    ...
    {
        $i = DataKeyword::whereIn('id',$request->ids)->select(['id','name'])->get();;
        $dirty = [];
        if (count($i) > 0) {
            ...
            if (count($dirty) > 0) {
                ...
                try {
                    ...
                    $this->saveTractAction($this->getTrackData(title: "DataKeyword list was updated by ".$request?->auth?->name, request: $request));
                    DB::commit();
                    return $this->response(['type' => 'success','data' => $data]);
                } catch (\Exception $e) {
                    DB::rollback();
                    $this->saveError($this->getSystemError(['name' => 'DataKeyword_bulk_update_error']), $e);
                    return $this->response(['type' => 'wrong', 'lang' => 'server_wrong']);
                }
            } else {
                return $this->response(['type' => 'noUpdate', 'title' =>  '<span class="text-success"> '.pxLang($request->lang,'','common.no_change').'  </span>']);
    ...
    {
        $errors = [];
        $i = DataKeyword::whereIn('id',$request->ids)->select(['id'])->get();
        if (count($i) > 0) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: "DataKeyword list was deleted by ".$request?->auth?->name, request: $request));
                DB::commit();
                return $this->response(['type' => 'success',"data"=>$data]);
            } catch (\Exception $e) {
                DB::rollback();
